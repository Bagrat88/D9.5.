...
from celery.utils.term import YELLOW, RED, BLUE
...
SECRET_KEY = 'django-insecure-z27m!x%5g3lx9&ikpko=alh&t59-5)u=3vi#ubu-w++%uy_m%%'
...
LOGGING = {
    ...
    'formatters': {
        ...
        # },
        'verbose': {
            'format': f'{YELLOW}%(levelname)s {BLUE} %(asctime)s :: \
                       %(pathname)s %(module)s %(process)d %(thread)d %(message)s'
        },
        'error_format': {
            'format': f'{RED}%(levelname)s :{BLUE}: %(asctime)s \
                       %(pathname)s %(module)s %(process)d %(thread)d %(message)s %(exc_info)s'
        },
        'info_format': {
            'format': f'{YELLOW}%(levelname)s :{BLUE}: %(asctime)s %(module)s %(message)s'
        },
        'error_file_format': {
